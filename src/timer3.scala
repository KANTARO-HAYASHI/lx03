...
case  class TimerWorld3(sec: Int, subsec: Int) extends World() {
  /**
   * draw は画面の描画を担当する関数。
  ...
  def draw(): Boolean = {
    ...
    // 画面に経過時間を表示
    canvas.drawString(Pos(0, canvas.height), s"${sec}")
    true
  }
   ...
   **/
  def tick():                World = {
    if (subsec + 1 == 100) { TimerWorld3(sec + 1, 0) }
    else { TimerWorld3(sec, subsec + 1) }
  }
   ...
   * この例では積極的には用いない。
   **/
  def click(p: Pos):         World = { TimerWorld3(sec, subsec) }
  def keyEvent(key: String): World = { TimerWorld3(sec, subsec) }
}

object TimerApp3 extends App {
  // 初期ワールドの構成。ここから世界が始まる。
  TimerWorld3(0, 0).bigBang(50, 50, 0.01)
}
